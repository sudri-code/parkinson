...
Clean-room skeleton (stdin parse, extract_conversation_context, subprocess
spawn) written against docs/architecture.ru.md §Hook System. Project
detection and spawn-chain composition ported from the author's prior work.
...
# Recursion guard: if we were spawned by flush.py (Agent SDK → hooks),
# exit immediately.
...
def extract_conversation_context(transcript_path: Path) -> tuple[str, int]:
    """Parse JSONL transcript into markdown-ish dump of the last N turns."""
    turns: list[str] = []
...
def main() -> None:
    ...
    synth_script = SCRIPTS_DIR / "synthesize_instincts.py"
    if synth_script.exists():
        try:
            ...
            logging.info("Spawned synthesize_instincts.py")
        except Exception as e:
            logging.error("Failed to spawn synthesize_instincts.py: %s", e)

    shield_script = SCRIPTS_DIR / "agentshield_run.py"
    if shield_script.exists():
        try:
            subprocess.Popen(
                [
                    "uv", "run", "--directory", str(INSTALL_DIR),
                    "python", str(shield_script),
                ],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
                creationflags=creation_flags,
            ...
            logging.info("Spawned agentshield_run.py")
        except Exception as e:
            logging.error("Failed to spawn agentshield_run.py: %s", e)
