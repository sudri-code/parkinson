...
Формат ответа — markdown-секции с bullet-пунктами. Не используй tools,
не пиши файлы.

ЖЁСТКОЕ ПРАВИЛО ПРОТИВ ВЫДУМКИ РЕПЛИК. Никогда не сочиняй и не «реконструируй»
сообщения пользователя. Если хочешь процитировать пользователя дословно
в формате `**User:** ...` — текст должен **буквально** присутствовать в
дампе ниже. Если буквальной цитаты нет, либо опусти реплику, либо
перескажи действие пользователя в третьем лице («пользователь попросил
продолжить», «пользователь отверг подход»). То же для `**Assistant:**`:
не додумывай ходы, которых нет в дампе. Стилистически правдоподобная,
но отсутствующая в источнике реплика — самая опасная категория ошибок,
потому что она искажает память пользователя о том, что он на самом деле
говорил и решал.

ВАЖНО про FLUSH_OK. Верни ровно строку `FLUSH_OK` (и больше ничего)
ТОЛЬКО в одном случае: контекст полностью пустой, либо это исключительно
greeting/farewell без какой-либо технической работы. Во всех остальных
случаях — даже если разговор кажется «рутинным» (правка кода, git-коммит,
прогон тестов, диагностика, обсуждение архитектуры) — обязательно извлеки
минимум **Context** + одну-две релевантные секции из скелета. Технические
сессии — основной материал базы знаний; не отбрасывай их как «без
...
**Key Exchanges:** (пересказ ключевых развилок в третьем лице — что
пользователь попросил/отверг/уточнил и как ассистент отреагировал. БЕЗ
прямых цитат и БЕЗ формата `**User:** ...` / `**Assistant:** ...`.
Дословные цитаты допустимы только если фраза буквально присутствует в
дампе И несёт смысл, который пересказ теряет — тогда обычным inline-
цитированием в кавычках, не отдельной репликой.)
- ...
...
def main(argv: list[str]) -> int:
    ...
    if body is not None:
        ...
        if _should_autocompile(daily_path):
            _spawn_compile()
    # When body is None, _call_agent has already logged the precise reason
    # (empty response / explicit FLUSH_OK / SDK exception).
